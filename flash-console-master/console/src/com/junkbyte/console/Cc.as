...
package com.junkbyte.console {
	...
	public class Cc{
		...
		public static function start(container:DisplayObjectContainer, password:String = ""):void{
			...
			remoting = true;
			commandLine = true; // Show command line
		}
		/**
			...
			if(_console){
				...
				if(display && display.stage){
					start(display.stage, password);
			 		...
			 		_console = new Console(password, config);
			 		// if no parent display, console will always be hidden, but using Cc.remoting is still possible so its not the end.
					config.commandLineAllowed = true; // enable advanced (but security risk) features.
					config.tracing = true; // Also trace on flash's normal trace
				//	config.remotingPassword = "thomas2015"; // Just so that remote don't ask for password
		...
		public static function changeCosoleV(v:Boolean):void{
			if(_console)
				_console.changeCosoleV(v);
		}
		/**
